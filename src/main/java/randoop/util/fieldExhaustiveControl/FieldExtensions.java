...
public class FieldExtensions {
	...
	private HashMap<String,HashMap<Integer,Collection<Integer>>> objectFieldExtensions = new HashMap<String,HashMap<Integer,Collection<Integer>>>();
	private HashMap<String,HashMap<Integer,Collection<Object>>> primitiveFieldExtensions = new HashMap<String,HashMap<Integer,Collection<Object>>>();
	...
	public boolean add(Object newObject) {
		...
		if (HeapDumper.isPrimitive(newObject)) return false;
		Tuple<HashMap<String,HashMap<Integer,Integer>>,HashMap<String,HashMap<Integer,Object>>> exts = HeapDumper.dump(newObject, this.maxDepth, this.maxArrayLength, this.ignoreList);
		HashMap<String,HashMap<Integer,Integer>> newObjectExts = exts.getFirst();
		HashMap<String,HashMap<Integer,Object>> newPrimitiveExts = exts.getSecond();
		boolean addedNewFieldValue = false;
		for (String key: newObjectExts.keySet()) {
			if (!objectFieldExtensions.containsKey(key)) {
				HashMap<Integer,Collection<Integer>> extsForKey = new HashMap<Integer,Collection<Integer>>();
		...
		for (String key: newPrimitiveExts.keySet()) {
			if (!primitiveFieldExtensions.containsKey(key)) {
				HashMap<Integer,Collection<Object>> extsForKey = new HashMap<Integer,Collection<Object>>();
