...
public class ExtensionsCollectorInOutVisitor implements ExecutionVisitor {
	...
	public void visitBeforeStatement(ExecutableSequence sequence, int i) {
		...
		// Only use method names while checking if an input is new according to field extensions
	...
	private String getOperationClass(TypedOperation op) {
		...
		String cls = typOp.getDeclaringType().getName();
		return cls;
...
//		return classNameNoGenerics;
	...
	public void visitAfterStatement(ExecutableSequence sequence, int i) {
		...
		if (statementResult instanceof NormalExecution) {
			...
			// because in Java we don't have runtime information about generic types
			if (sequence.sequence.size() == 1 && op.isConstructorCall())
				newOutput = true;
		}
	}
