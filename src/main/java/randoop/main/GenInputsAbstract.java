...
public abstract class GenInputsAbstract extends CommandHandler {
	...
	/**
	 * The fully-qualified name of a class to test. This class is tested in addition
	 * to any specified using <code>--classlist</code>, and must be accessible from
	 * the package of the tests (set with <code>--junit-package-name</code>).
	 */
	///////////////////////////////////////////////////////////////////
	 ...
	 * on a separate line. See an
	 * <a href= "https://randoop.github.io/randoop/manual/class_list_example.txt">
	...
	public static String method_classification_res = "classification-res.txt";
	 ...
	 *
	 * In the file, each each method under test is specified on a separate line. The
	 * list of methods given by this argument augment any methods derived via the
	 * <code>--testclass</code> or <code>--classlist</code> option.
	 *
	 * <p>
	 * A constructor line begins with <code>"cons :"</code> followed by the
	 * classname, the string <code>&lt;init&gt;</code> and the constructor's
	 * parameter types, enclosed in parentheses. Methods are specified in a similar
	 * way. For example:
	 * </p>
	 *
	 * <pre class="code">
	 ...
	 * <p>
	 * See an
	 * <a href= "https://randoop.github.io/randoop/manual/method_list_example.txt">
	 * example</a>.
	 * </p>
	 */
	 ...
	 * generation. Otherwise, Randoop includes all public fields of a visible class.
	 */
	 ...
	 * restricts the tests to only use public members even if the class is a member
	 * of the same package as the generated tests.
	 */
	 ...
	 * <code>-javaagent:exercised_agent.jar</code> to instrument the classes. A test
	 * is output only if it exercises at least one of the class names in the file. A
	 * test exercises a class if it executes any constructor or method of the class,
	 * directly or indirectly (the constructor or method might not appear in the
	 * source code of the test). Included classes may be abstract.
	 */
	 ...
	 * <p>
	 * Will completely disable output when used with
	 * <code>--no-error-revealing-tests</code>. Be aware that restricting output can
	 * result in long runs if the default values of <code>--inputlimit</code> and
	 * <code>--timelimit</code> are used.
	 * </p>
	 */
	 ...
	 * exception, it should continue to throw an exception of the same type). Tests
	 * without assertions can be used to exercise the code, but they do not enforce
	 * any particular behavior, such as values returned.
	 */
	...
	/**
	 * Ignore the situation where a code sequence that previously executed normally
	 * throws an exception when executed as part of a longer test sequence. If true,
	 * the sequence will be classified as invalid. If false, Randoop will halt with
	 * information about the sequence to aid in identifying the issue.
	 *
	 * <p>
	 * Use of this option is a last resort. Flaky tests are usually due to calling
	 * Randoop on side-effecting or nondeterministic methods, and a better solution
	 * is not to call Randoop on such methods.
	 * </p>
	 */
	 ...
	 *
	 * Test generation stops when either the time limit (--timelimit) is reached, OR
	 * the number of generated sequences reaches the input limit (--inputlimit), OR
	 * the number of error-revealing and regression tests reaches the output limit
	 * (--outputlimit).
	 *
	 ...
	 * generation stops when either the time limit (--timelimit) is reached, OR the
	 * number of generated sequences reaches the input limit (--inputlimit), OR the
	 * number of error-revealing and regression tests reaches the output limit
	 * (--outputlimit).
	 *
	 * <p>
	 ...
	 * However, the current implementation means that the actual number of tests in
	 * the output can still be substantially smaller than this limit.
	 * </p>
	 *
	 * <p>
	 * This limit will have no effect if there is no output, which occurs when using
	 ...
	 * generated sequences reaches the input limit (--inputlimit), OR the number of
	 * error-revealing and regression tests reaches the output limit
	 * (--outputlimit). The number of tests output will be smaller than then number
	 * of test candidates generated, because redundant and illegal tests may be
	 * discarded.
	 *
	 * <p>
	 ...
	 * either <code>--dont-output-tests</code> or
	 * <code>--no-error-revealing-tests</code> together with
	 * <code>--no-regression-tests</code>. Otherwise the <code>--outputlimit</code>
	 * command-line option is usually more appropriate.
	 * </p>
	 */
	 ...
	 *
	 * For example, a null ratio of 0.05 directs Randoop to use <code>null</code> as
	 * an input 5 percent of the time when a non-<code>null</code> value of the
	 * appropriate type is available.
	 *
	 * Unless --forbid_null is true, a <code>null</code> value will still be used if
	 * no other value can be passed as an argument even if --null-ratio=0.
	 *
	 ...
	 * Strings longer than 65KB (or about 10,000 characters) may be rejected by the
	 * Java compiler, according to the Java Virtual Machine specification.
	 */
	 ...
	 *
	 * <p>
	 * A ratio of 0 results in tests where each value created within a test input is
	 * typically used at most once as an argument in a method call. A ratio of 1
	 * tries to maximize the number of times values are used as inputs to parameters
	 * within a test.
	 * </p>
	 */
	 ...
	 * this option is given, tests with fewer calls are given greater weight during
	 * its random selection. This has the overall effect of producing smaller JUnit
	 * tests.
	 * </p>
	 */
	 ...
	 *
	 * <p>
	 * Randoop stores previously-generated tests in a "component" set, and uses them
	 * to generate new tests. Setting this variable to a small number can sometimes
	 * result in a greater variety of tests generated during a single run.
	 * </p>
	 */
	 ...
	 * same as the package of a class under test, then package visibility rules are
	 * used to determine whether to include the class or class members in a test.
	 * Tests can be restricted to public members only by using the option
	 ...
	 * JUnit's reflective invocations can interfere with code instrumentation, such
	 * as by the DynComp tool. If that is a problem, then set this to false and
	 * Randoop will output tests that use direct method calls instead of reflection.
	 * The tests will execute methods and assertions, but won't be JUnit suites. The
	 * tests will include a <code>main</code> method.
	 */
	 ...
	 * deterministic: running it twice will produce the same test suite. If you want
	 * to produced multiple different test suites, run Randoop multiple times with a
